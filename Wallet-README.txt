...
On OSX its `~/Library/Ethereum/XDC.ipc` on linux `~/.ethereum/XDC.ipc` and on windows it uses a named pipe, which doesn't need to be renamed.

Example:

    $ XDC --datadir /my/chain/ --networkid 23 --ipcpath ~/Library/Ethereum/XDC.ipc
