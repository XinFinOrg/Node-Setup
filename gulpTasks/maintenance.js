...
gulp.task('update-nodes', cb => {
  const clientBinariesGeth = clientBinaries.clients.XDC;
  const localGethVersion = clientBinariesGeth.version;
  const newJson = clientBinaries;
  const XDC = newJson.clients.XDC;

  // Query latest XDC version
  got('https://api.github.com/repos/ethereum/go-ethereum/releases/latest', {
    ...
    .then(tagName => {
      ...
      // Compare to current XDC version in clientBinaries.json
      if (cmp(latestGethVersion, localGethVersion)) {
        XDC.version = latestGethVersion;
          ...
          .then(hash => {
            ...
            got(
              ...
              .then(response => {
                ...
                _.keys(XDC.platforms).forEach(platform => {
                  _.keys(XDC.platforms[platform]).forEach(arch => {
                    // Update URL
                    let url = XDC.platforms[platform][arch].download.url;
                    url = url.replace(
                      /\d+\.\d+\.\d+-[a-z0-9]{8}/,
                      `${latestGethVersion}-${hash}`
                    );
                    XDC.platforms[platform][arch].download.url = url;

                    // Update bin name (path in archive)
                    let bin = XDC.platforms[platform][arch].download.bin;
                    bin = bin.replace(
                      /\d+\.\d+\.\d+-[a-z0-9]{8}/,
                      `${latestGethVersion}-${hash}`
                    );
                    XDC.platforms[platform][arch].download.bin = bin;

                    // Update expected sanity-command version output
                    XDC.platforms[platform][
                      arch
                    ...
                    blobs.forEach(blob => {
                      if (
                        String(blob.Name) ===
                        _.last(
                          XDC.platforms[platform][arch].download.url.split('/')
                        )
                      ) {
